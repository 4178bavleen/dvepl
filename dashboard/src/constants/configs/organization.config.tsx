...
export const companiesConfig = {
  ...
  columns: [
    ...
    { 
      ...
      cell: ({ getValue }) => {
        const val = getValue();
        return (
          <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${val ? 'bg-success/15 text-success' : 'bg-muted-foreground/15 text-muted-foreground'}`}>
            {val ? 'Active' : 'Inactive'}
          </span>
        );
      }
  ...
  fields: [
    { name: 'name', label: 'Company Name', type: 'text', placeholder: 'Enter company name', required: true },
    { name: 'gst', label: 'GSTIN', type: 'text', placeholder: '27AAAAA1111A1Z1' },
    { name: 'pan', label: 'PAN', type: 'text', placeholder: 'AAAAA1111A' },
    { name: 'email', label: 'Email Address', type: 'text', placeholder: 'info@company.com' },
    { name: 'phone', label: 'Phone Number', type: 'text', placeholder: '+91 22 5555 1234' },
    { name: 'address', label: 'Registered Address', type: 'textarea', placeholder: 'Enter address details' },
    { name: 'isActive', label: 'Active', type: 'checkbox' }
  ] as any[],
  statsCards: (data: Company[]) => [
    { label: 'Total Companies', value: data.length },
    { label: 'Active Companies', value: data.filter(c => c.isActive).length, change: '100%', trend: 'up' as const },
...
export const branchesConfig = {
  ...
  defaultFormValues: { companyId: '', name: '', code: '', address: '', city: '', state: '', pincode: '', isActive: true },
...
export const departmentsConfig = {
  ...
  fields: [
    { name: 'branchId', label: 'Branch Location', type: 'select', options: [
...
export const teamsConfig = {
  ...
  statsCards: (data: Team[]) => [
    { label: 'Total Teams', value: data.length }
  ]
};
...
export const costCentersConfig = {
  ...
  fields: [
    ...
    { name: 'departmentId', label: 'Department Link', type: 'select', options: [
      { label: 'Sales & Marketing', value: 'dept-1' },
      { label: 'Human Resources', value: 'dept-2' },
      { label: 'Finance & Accounts', value: 'dept-3' }
    ] }
  ...
  statsCards: (data: CostCenter[]) => [
    ...
    { label: 'Total ERP Budget', value: `₹${data.reduce((sum, item) => sum + Number(item.budget || 0), 0).toLocaleString()}` }
  ]
};
