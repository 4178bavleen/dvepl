...
export function CostCenterPage() {
  ...
  const [formValues, setFormValues] = useState<Record<string, unknown>>({ code: "", name: "", budget: "0", departmentId: "" });
  const [editingRecord, setEditingRecord] = useState<CostCenter | null>(null);
  const [viewingRecord, setViewingRecord] = useState<CostCenter | null>(null);
  const [isFormOpen, setIsFormOpen] = useState(false);
  const [errors, setErrors] = useState<Record<string, string>>({});
  const [records, setRecords] = useState<CostCenter[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [deleteConfirmOpen, setDeleteConfirmOpen] = useState(false);
  const [recordToDelete, setRecordToDelete] = useState<CostCenter | null>(null);
  ...
  const canCreate = canPerformPageAction(currentUser?.actionPermissions, "cost_centers", "create");
  const canEdit = canPerformPageAction(currentUser?.actionPermissions, "cost_centers", "edit");
  const canDelete = canPerformPageAction(currentUser?.actionPermissions, "cost_centers", "delete");

  const defaultFormValues = { code: "", name: "", budget: "0", departmentId: "" };
  ...
  const fields: Array<{ name: string; label: string; type: "text" | "number" | "select"; placeholder?: string; required?: boolean }> = [
    { name: "code", label: "Cost Center Code", type: "text", placeholder: "CC-MUM-SALES", required: true },
    { name: "name", label: "Cost Center Name", type: "text", placeholder: "Marketing Overhead", required: true },
    { name: "budget", label: "Budget Allocation (INR)", type: "number", placeholder: "500000", required: true },
    { name: "departmentId", label: "Department Link", type: "select" },
  ];
  const columns: ColumnDef<CostCenter>[] = [
    { accessorKey: "code", header: sortableHeader("Code") },
    { accessorKey: "name", header: sortableHeader("Name") },
    { accessorKey: "budget", header: sortableHeader("Annual Budget"), cell: ({ getValue }) => `₹${Number(getValue() || 0).toLocaleString()}` },
    {
      accessorKey: "departmentId", header: "Department",
      cell: ({ row }) => {
        const dept = (row.original as any).department;
        return <span className="text-sm text-foreground">{dept?.name ?? "—"}</span>;
      },
    },
  ];
  ...
  const loadRecords = useCallback(async () => {
    setIsLoading(true);
    try { setRecords(await organizationApi.costCenters.list()); }
    catch (e: any) { toast.error(e.response?.data?.message ?? "Unable to load cost centers."); }
  ...
  const statsCards = useMemo(() => [
    ...
    { label: "Total ERP Budget", value: `₹${records.reduce((sum, item) => sum + Number(item.budget || 0), 0).toLocaleString()}` },
  ], [records]);

  const openCreate = () => { setEditingRecord(null); setFormValues(defaultFormValues); setErrors({}); setIsFormOpen(true); };
  const openEdit = (r: CostCenter) => { setEditingRecord(r); setFormValues({ ...defaultFormValues, ...r }); setErrors({}); setIsFormOpen(true); };
  ...
  const submitForm = async (e: React.FormEvent) => {
    ...
    try {
      if (editingRecord && organizationApi.costCenters.update) await organizationApi.costCenters.update(editingRecord.id, result.data as Record<string, unknown>);
      else await organizationApi.costCenters.create(result.data as Record<string, unknown>);
      await loadRecords(); setIsFormOpen(false); toast.success("Cost Center saved successfully.");
    } catch (e: any) { toast.error(e.response?.data?.message ?? "Unable to save cost center."); }
  ...
  return (
    <div className="space-y-6 p-4 sm:p-6 lg:p-8">
      <nav aria-label="Breadcrumb" className="text-sm text-muted-foreground">
        <span className="text-foreground">Dashboard</span><span className="mx-2">/</span><span className="text-foreground">Cost Centers</span>
      </nav>
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">Cost Centers</h1>
          <p className="mt-1 text-sm text-muted-foreground">Manage cost centers.</p>
        </div>
        <div className="flex items-center gap-2">
          {canCreate && <Button onClick={openCreate} className="gap-2"><Plus className="size-4" /> Add Cost Center</Button>}
        </div>
      </div>
      ...
      <div className="relative max-w-md">
        ...
        <Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search cost centers..." className="pl-9" />
      </div>
      <GenericTable<any> columns={columns as any} data={filteredRecords} onView={setViewingRecord} onEdit={canEdit ? openEdit : undefined}
        onDelete={!canDelete ? undefined : (r) => { setRecordToDelete(r); setDeleteConfirmOpen(true); }} isLoading={isLoading} freezeActions={true} storageKey="costCenters" />
      ...
      <Dialog open={isFormOpen} onOpenChange={setIsFormOpen}>
        <DialogContent className="w-full max-w-lg max-h-[90vh] overflow-y-auto p-0">
          <DialogHeader className="bg-gradient-to-br from-primary/5 via-background to-transparent border-b p-6">
            ...
            <DialogTitle className="text-xl font-bold tracking-tight text-foreground">{editingRecord ? "Edit Cost Center" : "Add Cost Center"}</DialogTitle>
          ...
          <form onSubmit={submitForm} className="flex-1 flex flex-col justify-between overflow-y-auto">
            ...
            <div className="bg-muted/30 border-t p-4 px-6 flex items-center justify-end gap-3 sticky bottom-0 bg-background">
              ...
              <Button type="submit" className="shadow-xs hover:opacity-90">{editingRecord ? "Save changes" : "Create Cost Center"}</Button>
            </div>
          </form>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={Boolean(viewingRecord)} onOpenChange={(o) => !o && setViewingRecord(null)}>
        <DialogContent className="w-[95vw] max-w-3xl max-h-[85vh] overflow-y-auto p-0">
          {viewingRecord && (
            <>
              <DialogHeader className="px-6 pt-6"><DialogTitle>Cost Center Overview</DialogTitle><DialogDescription>View complete cost center details.</DialogDescription></DialogHeader>
      ...
      <ConfirmDialog open={deleteConfirmOpen} onOpenChange={setDeleteConfirmOpen} title="Move Cost Center to Recycle Bin?"
        description="This cost center will be moved to the Recycle Bin. You can restore it anytime from Settings → Recycle Bin." confirmText="Move to Bin"
        onConfirm={async () => {
          if (!recordToDelete) return; setIsLoading(true);
          try { if (organizationApi.costCenters.remove) { await organizationApi.costCenters.remove(recordToDelete.id); await loadRecords(); } toast.success("Cost Center deleted successfully."); }
          catch (e: any) { toast.error(e.response?.data?.message ?? "Unable to delete cost center."); }
