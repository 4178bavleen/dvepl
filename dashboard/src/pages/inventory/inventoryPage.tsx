                        ...
                        <div className="flex border-b gap-1 bg-muted/20 p-1 rounded-lg max-w-fit">
                            {[
                                ...
                                <button
                                    key={tab.id}
                                    onClick={() => setActiveTab(tab.id as any)}
                                    className={`px-4 py-1.5 rounded-md text-sm font-semibold tracking-wide transition-all ${activeTab === tab.id
                                            ? "bg-card text-foreground shadow-sm border border-border"
                                            : "text-muted-foreground hover:text-foreground"
                                        }`}
                                >
                                                ...
                                                {filteredItems.length === 0 ? (
                                                    ...
                                                    filteredItems.map((item) => (
                                                        <tr key={item.id} className="hover:bg-muted/10">
                                                            ...
                                                            <td className="p-4">
                                                                <span
                                                                    className={`text-[10px] font-bold px-2 py-0.5 rounded-full border uppercase tracking-wider ${item.type === "raw"
                                                                            ? "bg-blue-500/10 text-blue-500 border-blue-500/20"
                                                                            : "bg-emerald-500/10 text-emerald-500 border-emerald-500/20"
                                                                        }`}
                                                                >
                                                            ...
                                                            <td
                                                                className={`p-4 text-right font-bold ${item.currentStock === 0
                                                                        ? "text-rose-500"
                                                                        : item.reorderLevel > 0 &&
                                                                            item.currentStock <= item.reorderLevel
                                                                            ? "text-amber-500"
                                    ...
                                    <div className="p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                        {filteredItems.length === 0 ? (
                                            ...
                                            filteredItems.map((item) => (
                                                <div
                                                    key={item.id}
                                                    className={`p-4 rounded-xl border bg-card flex flex-col justify-between shadow-sm hover:shadow transition-all ${item.currentStock === 0
                                                            ? "border-rose-500/30 bg-rose-500/[0.01]"
                                                            : item.reorderLevel > 0 &&
                                                                item.currentStock <= item.reorderLevel
                                                                ? "border-amber-500/30 bg-amber-500/[0.01]"
                                        ...
                                        {movements.length === 0 ? (
                                            ...
                                            movements.map((m) => (
                                                <tr key={m.id} className="hover:bg-muted/10 text-xs">
                                                    ...
                                                    <td className="p-4">
                                                        <span
                                                            className={`text-[9px] font-bold px-1.5 py-0.5 rounded border uppercase tracking-wider ${m.itemType === "raw"
                                                                    ? "bg-blue-500/10 text-blue-500 border-blue-500/20"
                                                                    : "bg-emerald-500/10 text-emerald-500 border-emerald-500/20"
                                                                }`}
                                                        >
                                                    ...
                                                    <td className="p-4">
                                                        <span
                                                            className={`text-[9px] font-bold px-1.5 py-0.5 rounded border uppercase tracking-wider ${m.movementType === "IN"
                                                                    ? "bg-emerald-500/10 text-emerald-500 border-emerald-500/20"
                                                                    : m.movementType === "OUT"
                                                                        ? "bg-rose-500/10 text-rose-500 border-rose-500/20"
                                                                        : m.movementType === "RETURN"
                                        ...
                                        {(["IN", "OUT", "ADJUST", "RETURN"] as const).map(
                                            (type) => (
                                                <button
                                                    ...
                                                    type="button"
                                                    onClick={() => setStockMovType(type)}
                                                    className={`flex-1 text-center py-1 text-xs font-bold rounded ${stockMovType === type
                                                            ? "bg-card text-foreground shadow border border-border/80"
                                                            : "text-muted-foreground hover:text-foreground"
                                                        }`}
                                                >
